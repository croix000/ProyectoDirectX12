...
void Game::Update(DX::StepTimer const& timer)
{
	...
	float m_view_x = m_view._41;
	float m_view_y = m_view._42;
	float m_view_z = m_view._43;
    ...
    for (int i = 0; i < m_objects.size();i++) {
        ...
        for (auto &obj : objInstances) {
			...
			float d = x * x + y * y + z * z;
				...
				if (!m_objects[i][pos].hide && abs(m_view_x - x) < 1 && abs(m_view_y - y) < 1 && m_controller->m_leftClicked) {
					OutputDebugString(L"Should add score "+ obj.score);
					OutputDebugString(L"\n Should remove obj\n");

					m_score++;
					m_objects[i][pos].hide = true;
					

				}
				if(!m_objects[i][pos].hide)
					{
...
void Game::RenderUI()
{
    ...
    wchar_t text[100]; 
	size_t lenText = swprintf_s(text, 100, L"");
	if(m_score < instancedObj)
		lenText = swprintf_s(text,100,L"Score: %d /%d",m_score ,instancedObj);
	else
		lenText = swprintf_s(text, 100, L"YOU WIN");
